...
const replaceSenderId = replace(/SENDER_ID/)
...
const WELCOME =
  'Welcome! @RECEIVER_ID is now part of the elect circle of chocolaty calory connoisseurs at XING. @SENDER_ID'
const ERROR_FRIEND =
  '@RECEIVER_ID ??? Vouch for someone by tweeting their twitter name and the word "friend" to me.'

const getMessage = curry((msg, receiverId, senderId = null) => {
  if (!receiverId) return null
  if (senderId) {
    return replaceSenderId(senderId, replaceReceiverId(receiverId, msg))
  } else {
